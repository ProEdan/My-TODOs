...
class TestTodoModel(unittest.TestCase):
    ...
    def test_todo_creation(self) -> None:
        ...
        self.assertEqual(self.todo.status, TodoStatus.ACTIVE)
...
class TestSearchEngine(unittest.TestCase):
    ...
    def test_filter_by_priority(self) -> None:
        """测试优先级过滤。"""
        results = self.engine.filter_by_priority(Priority.HIGH)
    ...
    def test_sort_by_priority(self) -> None:
        ...
        self.assertEqual(results[0].priority, Priority.HIGH)
...
class TestExporter(unittest.TestCase):
    ...
    def test_import_from_json(self) -> None:
        """测试从JSON导入。"""
        # 创建一个简单的JSON字符串
        json_str = '[{"id":"1","text":"测试"}]'
        with open("test_import.json", "w") as f:
            f.write(json_str)

        try:
            # 注意: 实际的 import_from_json 期望文件路径
            # 这里只是演示测试结构
            pass
        finally:
            import os
